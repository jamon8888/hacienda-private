...
// alef:hash:ec028586395454412092974d66a8e474c2278bec337cffe0d6c052d8d1882b90
...
pub mod granite_embed;

pub use granite_embed::*;
...
impl WasmNerConfig {
    ...
    #[wasm_bindgen(setter)]
    pub fn set_adapter(&mut self, value: Option<String>) {
        self.adapter = value;
    }
...
/// A `xberg_rag.Embedder` backed by `embed_texts`.
///
...
#[derive(Clone)]
#[wasm_bindgen]
pub struct WasmXbergEmbedder {
    pub(crate) inner: Arc<xberg::XbergEmbedder>,
}

#[wasm_bindgen]
impl WasmXbergEmbedder {
    ...
    #[allow(clippy::missing_errors_doc)]
    #[wasm_bindgen]
    pub fn new(config: WasmEmbeddingConfig) -> Result<WasmXbergEmbedder, JsValue> {
        let result = xberg::XbergEmbedder::new(config.into()).map_err(|e| JsValue::from_str(&e.to_string()))?;
        Ok(Self {
    ...
    pub fn from_preset(name: String) -> Result<WasmXbergEmbedder, JsValue> {
        let result = xberg::XbergEmbedder::from_preset(&name).map_err(|e| JsValue::from_str(&e.to_string()))?;
        Ok(Self {
            inner: Arc::new(result),
        })
    }
    ...
    pub fn embed_documents(&self, _texts: Vec<String>) -> Result<JsValue, JsValue> {
        Err(JsValue::from_str("Not implemented: embed_documents"))
    }

    #[allow(clippy::missing_errors_doc)]
    #[wasm_bindgen(js_name = "embedQuery")]
    pub fn embed_query(&self, text: String) -> Result<Vec<f32>, JsValue> {
        let result = self
            .inner
            .embed_query(&text)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
impl From<xberg::NerConfig> for WasmNerConfig {
    fn from(val: xberg::NerConfig) -> Self {
        Self {
            ...
            adapter: val.adapter.map(|v| v.to_string()),
            llm: val.llm.map(Into::into),
            custom_labels: val.custom_labels.into_iter().collect(),
        }
    }
