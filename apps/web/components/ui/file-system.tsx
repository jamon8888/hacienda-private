...
function FileSystemIconsView({ entries, onOpen, onSelect, renderFilePreview, selectedPath }: FileSystemViewProps) {
	...
	React.useLayoutEffect(() => {
		...
		const update = () => {
			...
			setColumnCount(
				Math.max(1, Math.floor((available + ICON_TILE_GAP_X) / (ICON_MIN_TILE_WIDTH + ICON_TILE_GAP_X))),
			);
		};
...
function FileSystemListView({
	...
	if (relativePaths.length === 0) {
		return (
			<FileSystemEmptyState label={fileFilter ? "No items match the active filters" : "This folder is empty"} />
		);
	}
