...
import { FileImageIcon, FileSpreadsheetIcon, FileUploadIcon, Upload01Icon } from "@hugeicons/core-free-icons";
	...
	const dropzone = (
		...
		>
			...
			<div className="space-y-1">
				...
				{rejectionMessage ? <div className="text-xs text-destructive">{rejectionMessage}</div> : null}
			</div>
		...
		<div className={cn("space-y-3", className)}>
			...
			{showFileList && files.length > 0 ? (
				<div className="rounded-xl border bg-background">
					{files.map((file) => (
						<div key={file.id} className="flex items-center gap-3 border-b px-3 py-2.5 last:border-b-0">
							<FileThumbnail
								file={{
									name: file.name,
									type: file.type,
							...
							<div className="rounded-full bg-muted px-2 py-1 text-xs text-muted-foreground">Ready</div>
						</div>
					))}
				</div>
			) : null}
