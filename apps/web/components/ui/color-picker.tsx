...
function rgbToHsl(red: number, green: number, blue: number): HslColor {
  ...
  return [normalizeHue(h * 360), clampNumber(s * 100, 0, 100), clampNumber(l * 100, 0, 100)];
}
...
function hslToHsv([hue, saturation, lightness]: HslColor): HsvColor {
  ...
  return [normalizeHue(hue), clampPercent(hsvSaturation * 100), clampPercent(value * 100)];
}
...
function hsvToHsl([hue, saturation, value]: HsvColor): HslColor {
  ...
  return [normalizeHue(hue), clampPercent(hslSaturation * 100), clampPercent(lightness * 100)];
}
...
function parseColor(value: string): HslColor | undefined {
  ...
  if (hslMatch?.[1]) {
    ...
    if (values.length >= 3) {
      return [normalizeHue(values[0]!), clampPercent(values[1]!), clampPercent(values[2]!)];
    }
  }
...
function ColorPickerTooltip({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <Tooltip>
...
export function ColorPickerPanel({
  ...
  const parsedColor = React.useMemo(() => parseColor(color) ?? parseColor("#111827")!, [color]);
  const parsedFieldColor = React.useMemo(() => hslToHsv(parsedColor), [parsedColor]);
  ...
  const [inputValue, setInputValue] = React.useState(() => formatColor(parsedColor, "hex"));
  ...
  function applyColor(nextColor: HslColor, nextFieldColor?: HsvColor) {
    ...
    const normalizedFieldColor = normalizeHsvColor(nextFieldColor ?? hslToHsv(normalizedColor));
  ...
  function updateColorField(event: React.PointerEvent<HTMLDivElement> | React.MouseEvent<HTMLDivElement>) {
  ...
  return (
    <div className={cn("space-y-1.5", className)}>
      <div className="flex justify-end">
        ...
        >
          <SelectTrigger size="sm" className="h-7 w-24 text-xs" aria-label="Color format">
            <SelectValue />
          </SelectTrigger>
      ...
      <input
        ...
        onChange={(event) =>
          applyColor(hsvToHsl([Number(event.target.value), fieldHsv[1], fieldHsv[2]]), [
          ...
          return (
            ...
            >
              {selected ? (
                <HugeiconsIcon
                  icon={CheckIcon}
                  className={cn("size-3.5", preset === "#ffffff" ? "text-foreground" : "text-white")}
                />
              ) : null}
            </button>
          );
