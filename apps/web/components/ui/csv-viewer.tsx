...
import type { DataEditorRef, GridCell, GridCellKind, GridColumn, Item, Theme } from "@glideapps/glide-data-grid";
...
export function CsvViewer({ className, data, search = false }: CsvViewerProps) {
  ...
  function stepZoom(direction: -1 | 1) {
    ...
    setZoom(ZOOM_OPTIONS[nextIndex] ?? zoom);
  }
            ...
            <div className="flex flex-none items-center gap-1">
              ...
              <ToolbarTooltip label="Zoom in">
                <Button
                  ...
                  size="icon-sm"
                  aria-label="Zoom in"
                  disabled={zoom >= (ZOOM_OPTIONS[ZOOM_OPTIONS.length - 1] ?? 1)}
                  onClick={() => stepZoom(1)}
                >
