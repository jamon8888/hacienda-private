...
const THUMBNAIL_SURFACE_CLASS_NAME = "relative overflow-hidden rounded-lg border border-border bg-background shadow-xs";
...
const splitterCollisionDetection: CollisionDetection = (args) => {
    ...
    return closestCenter({
      ...
      droppableContainers: args.droppableContainers.filter(
        (container) =>
          ...
          (container.data.current?.type === "page-dropzone" && container.data.current?.isEmpty),
      ),
    });
  }
  ...
  if (dragType === "split") {
    return closestCenter({
      ...args,
      droppableContainers: args.droppableContainers.filter((container) => container.data.current?.type === "split"),
    });
  }
...
export function createInitialSplits(pageCount: number) {
  ...
  const pages = Array.from({ length: pageCount }, (_, index) => `page-${index + 1}` as PageId);
  const chunkSize = Math.max(1, Math.ceil(pageCount / 3));

  return Array.from({ length: Math.ceil(pageCount / chunkSize) }, (_, index) => {
...
function movePageToGroup({
  ...
  return groups.map((group) => {
    const pagesWithoutDragged = group.pages.filter((pageId) => pageId !== draggedPageId);
...
function SplitGroupDropzone({ children, group }: { children: React.ReactNode; group: SplitGroup }) {
...
function PageThumbnail({
  ...
  return (
    <button
      ref={setNodeRef}
      type="button"
      className={cn(THUMBNAIL_BUTTON_CLASS_NAME, "shrink-0", isDragging && "opacity-0")}
      style={{
    ...
    <section className="w-full rounded-lg border bg-background">
      <div className="flex items-center justify-between gap-3 border-b p-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <div className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">{group.pages.length}</div>
          <Button
            type="button"
            variant="ghost"
            size="icon-sm"
...
export function DocumentSplits({
  ...
  const [activeSplitGroupId, setActiveSplitGroupId] = React.useState<string | null>(null);
  const [activeSplitGroupWidth, setActiveSplitGroupWidth] = React.useState<number | undefined>();
  ...
  const handleDragStart = React.useCallback(
    (event: DragStartEvent) => {
      ...
      if (dragType === "split") {
        ...
        setActiveSplitGroupId((event.active.data.current?.groupId as string | undefined) ?? null);
  ...
  const handleDragEnd = React.useCallback(
    (event: DragEndEvent) => {
      if (event.active.data.current?.type === "split" && event.over) {
        ...
        const draggedIndex = splits.findIndex((group) => group.id === activeGroupId);
  ...
  const activeSplitGroup = activeSplitGroupId ? splits.find((group) => group.id === activeSplitGroupId) : null;
              ...
              {splits.map((group) => (
                <SortableSplitGroupCard
                  ...
                  onRemove={() => {
                    ...
                    onSplitsChange(splits.filter((split) => split.id !== group.id));
                  }}
                  onSelectPage={onSelectPage}
                />
              ))}
