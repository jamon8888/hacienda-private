...
**Goal:** Warm the E5 embedding model and GLiNER PII model in the background as soon as the app loads, show a status pill in the app shell, gate the two real model-dependent actions in the app (search and document ingest), and cache all four fetched assets so this only takes real time on a genuine first visit.
...
- **Revision note (superseded scope correction):** an earlier draft of this plan, written before PR #23 (`feat(web): lawyer-grade enhancement`) was pulled into `main`, found that `FolderView.tsx` didn't call `ingestFolder()` yet and scoped ingest-gating out accordingly. PR #23 is now on `main` and wires up `FileDropzone` → `ingestFolder()` for real (see Task 8) — so **this plan now gates both search and ingest**, matching the original approved spec exactly. There is no remaining scope reduction on this point.
- **Baseline reality check (read before touching `embed.ts` / `ner.ts` / `constants.ts`):** PR #23 changed these files substantially beyond anything this plan anticipated. Every task below is written against the *current* content — do not assume the snippets in an older copy of this plan match what's on disk:
  - `constants.ts` no longer exports `GLINER_TOKENIZER_URL`. It now exports `GLINER_TOKENIZER_REPO_ID = "gliner-pii"` (a bare repo id, not a URL) — `@xenova/transformers`' `AutoTokenizer.from_pretrained()` always appends `/tokenizer.json` itself, so passing a full URL there double-appended into a broken, "remote-looking" path. The bare id resolves via `env.localModelPath` (set in `ner.ts`'s `disableRemoteModels()`) instead.
  - `embed.ts`'s `getSession()` now imports `onnxruntime-web/wasm` (not plain `onnxruntime-web`) and sets `ort.env.wasm.wasmPaths = "/ort/"` — both are real fixes for a worker-corruption bug and a broken webpack asset-URL auto-detection, not stylistic choices. It also filters `feeds` down to `session.inputNames` before calling `session.run()` (some e5 ONNX exports don't declare a `token_type_ids` input) and has several `console.log("DEBUG2 ...")` lines left in from e2e debugging. **Preserve all of this** — Task 2 edits the current file in place rather than replacing it wholesale.
  - `ner.ts` and `apps/web/lib/engine/adapter.ts` / `apps/web/lib/engine/index.ts` also gained real new code (an `openVault`/`rehydrateSpanForUi` PII-reveal path, a `docId`/`vaultSalt` addition to the ingest context and mirror payload). Task 3 and Task 5 are written as targeted edits against the current file content for the same reason.
...
**Do not replace the whole file** — `embed.ts` currently has real, working fixes in it (a `/wasm`-only onnxruntime import to dodge a webgpu/jsep worker-corruption bug, an explicit `wasmPaths = "/ort/"`, `session.inputNames`-filtered feeds, and several `console.log("DEBUG2 ...")` lines from e2e debugging). Read the current file first, then make exactly these targeted edits, keeping everything else byte-for-byte as-is:

1. Change the top of the file from:
...
2. Add a `resetEmbedSession` export directly above the `getSession` function:
...
3. Rename `async function getSession(` to `export async function ensureEmbedSession(`, add an `onProgress` parameter, and replace the raw `fetch` + `arrayBuffer()` call with `cachedFetchBuffer` — i.e. change:
...
(the rest of that function — the `/wasm` import, `wasmPaths`, `InferenceSession.create` call, "DEBUG2 session created" log — is untouched.)

4. Remove the private `fetchJson` helper function entirely (it becomes dead code — `cachedFetchJson` from Task 1 replaces it), and in `getTokenizer()`, change:
...
5. In `embedOne()`, change the one call site from `getSession(scenario)` to `ensureEmbedSession(scenario)`:

```ts
	const [session, tok] = await Promise.all([ensureEmbedSession(scenario), getTokenizer()]);
```

Leave everything else in `embedOne()` (the `/wasm` import, the `session.inputNames`-filtered `feeds` construction, the pooling/normalization math) exactly as it is. Leave `embedChunks`/`embedQuery` untouched — they already call `embedOne`, not `getSession`, so they need no changes.
...
**Do not replace the whole file** — the current `ner.ts` already has a real fix in it: `constants.ts` no longer exports `GLINER_TOKENIZER_URL` (only `GLINER_TOKENIZER_REPO_ID`), and `disableRemoteModels()` sets `env.localModelPath` so the bare repo id resolves correctly (see the Global Constraints note above for why). Read the current file first, then make exactly these targeted edits:
...
(keep this as a second `import` line right after the `./constants` one — do not touch the existing `env.localModelPath` line inside `disableRemoteModels()`.)

2. Add a `resetPiiModel` export directly above the `getModel` function:
...
3. Rename `async function getModel(` to `export async function ensurePiiModel(`, add an `onProgress` parameter, flip `useBrowserCache` to `true`, and pre-fetch the model bytes through the scoped override instead of letting gliner fetch them directly — i.e. change:
...
Above this function, extend (don't replace) the existing "Local tokenizer loading" comment block with one more paragraph explaining the model-binary caching, since the existing comment only covers the tokenizer:
...
4. In `detectPii()`, change the one call site from `getModel(scenario)` to `ensurePiiModel(scenario)`. Everything else in `detectPii` (the inference call, the result mapping) is untouched.
...
In `packages/wasm-pipeline/src/index.ts`, add this at the end of the file (after the existing `export { detectCapabilities, ... }` / `export { selectScenario, ... }` / `export { API_BASE, ... }` lines):
...
`apps/web/lib/engine/adapter.ts` currently ends with the `rehydrateSpanForUi` function (added by PR #23 for the PII-reveal flow — unrelated to this plan, do not touch it). Add these two lines at the very end of the file, after `rehydrateSpanForUi`'s closing `}`:
...
`apps/web/lib/engine/index.ts` currently reads:
...
Change it to (adding `warmupModels` and the two warmup types — `rehydrateSpanForUi` and the existing type exports are untouched):
...
```tsx
// apps/web/app/folders/[id]/FolderView.test.tsx
import { describe, expect, it, vi } from "vitest";
import { render, screen } from "@testing-library/react";
...
4. Update the Search button — change:
...
Read the current file first. Make these four targeted edits, leaving the SHA-256 hashing, `createDocument`/`updateDocumentStatus` calls, and upload-progress rendering completely untouched:
...
2. Read the warmup stage, right after the existing `useAuth()` call:
...
3. Extend the `onFilesAccepted` guard and its dependency array — change:
...
and change the hook's dependency array from `[auth, folder, folderId, matterId, refresh]` to `[auth, folder, folderId, matterId, refresh, modelStage]`.

4. Extend the render branch from a 2-way to a 3-way conditional — change:
