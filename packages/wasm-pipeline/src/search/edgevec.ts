...
export class EdgeVecSearchStore implements SearchStore {
		...
		for (const item of items) {
			...
			if (this.hybridAvailable) {
				...
				const sparse = provided
					? {
							indices: item.sparseIndices as Uint32Array,
							values: item.sparseValues as Float32Array,
							dim: this.vocab.dim,
						}
	...
	async load(matterId: string): Promise<boolean> {
		...
		for (const chunk of ordered) {
			...
			if (hybridAvailable) {
				const indices =
					chunk.sparseIndices && chunk.sparseIndices.length > 0 ? chunk.sparseIndices : DUMMY_SPARSE_INDICES;
				const values =
					chunk.sparseValues && chunk.sparseValues.length > 0 ? chunk.sparseValues : DUMMY_SPARSE_VALUES;
