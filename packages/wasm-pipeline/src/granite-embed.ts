...
function workerFor(): Worker | undefined {
	...
	if (!worker) {
		worker = new Worker(new URL("./granite-embed-worker.ts", import.meta.url), { type: "module" });
		worker.onmessage = (event: MessageEvent) => {
			const message = event.data as { id: number; ok: boolean; error?: string; [key: string]: unknown };
...
function callWorker(message: Record<string, unknown>, transfer: Transferable[] = []): Promise<Record<string, unknown>> {
	...
	return new Promise((resolve, reject) => {
		pending.set(id, { resolve: (value) => resolve(value as Record<string, unknown>), reject });
...
export async function embedChunks(chunks: { text: string }[], onProgress?: (progress: FetchProgress) => void): Promise<Float32Array[]> {
	await ensureGraniteEmbedder(onProgress);
	if (directModel) return directModel.embedDocuments(chunks.map((chunk) => chunk.text)).map((vector) => new Float32Array(vector));
	const result = await callWorker({ op: "documents", texts: chunks.map((chunk) => chunk.text) });
