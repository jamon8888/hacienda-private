...
class Gliner2WorkerSession implements Gliner2Session {
	...
	loadBytes(weights: Uint8Array, tokenizer: Uint8Array, encoderConfig: Uint8Array): void {
		void this.request("load", [weights.buffer, tokenizer.buffer, encoderConfig.buffer], [weights.buffer, tokenizer.buffer, encoderConfig.buffer]);
	}

	extractNer(_text: string, _labels: string[], _threshold?: number): Array<{ start: number; end: number; text: string; label: string }> {
		throw new Error("GLiNER2 Worker sessions require asynchronous extraction");
	}

	async extract(text: string, labels: string[], threshold: number): Promise<Array<{ start: number; end: number; text: string; label: string }>> {
		return (await this.request("extract", [text, labels, threshold])) as Array<{ start: number; end: number; text: string; label: string }>;
	}
...
export async function detectGliner2(
	...
	const spans = model instanceof Gliner2WorkerSession
