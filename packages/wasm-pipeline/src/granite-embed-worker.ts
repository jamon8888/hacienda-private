...
self.onmessage = async (event: MessageEvent) => {
	...
	try {
		...
		if (message.op === "load") {
			if (!message.weights || !message.tokenizer || !message.config) throw new Error("Granite artifact bytes are missing");
			model.loadBytes(new Uint8Array(message.weights), new Uint8Array(message.tokenizer), new Uint8Array(message.config));
			self.postMessage({ id: message.id, ok: true, identity: model.identity() });
			return;
		}
		if (message.op === "documents") {
			self.postMessage({ id: message.id, ok: true, vectors: model.embedDocuments(message.texts ?? []) });
			return;
		}
		self.postMessage({ id: message.id, ok: true, vector: model.embedQuery(message.text ?? "") });
	} catch (error) {
