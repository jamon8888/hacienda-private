...
self.onmessage = async (
  event: MessageEvent<{ id: number; command: string; args: unknown[] }>,
) => {
  const { id, command, args } = event.data;
  try {
    if (command === "load") {
      const wasm =
        (await import("@xberg-io/xberg-wasm")) as unknown as Gliner2WasmModule;
      if (typeof wasm.Gliner2Model !== "function")
        throw new Error("GLiNER2 is unavailable in this WASM package");
      const bytes = await Promise.all(
        urls.map((url) => cachedFetchBuffer(url)),
      );
      for (let index = 0; index < bytes.length; index++) {
        if ((await sha256(bytes[index]!)) !== expectedSha256[index]) {
          throw new Error(
            `GLiNER2 artifact integrity check failed for ${urls[index]}`,
          );
        }
      }
      model = new wasm.Gliner2Model();
      model.loadBytes(
        ...(bytes.map((bytes) => new Uint8Array(bytes)) as [
          Uint8Array,
          Uint8Array,
          Uint8Array,
        ]),
    ...
    if (command !== "extract")
      throw new Error(`unknown GLiNER2 Worker command: ${command}`);
