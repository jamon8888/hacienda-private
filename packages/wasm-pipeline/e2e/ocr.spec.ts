...
test("tesseract OCR extracts non-empty text from a receipt image", async ({ harness }) => {
  const ok = await harness.evaluate(async () => {
    const { initWasm, extractDocument, firstDocument, defaultExtractionConfig, withTesseractOcr } = window.XbergPipeline;
    await initWasm();
    ...
    return firstDocument(result) != null;
  });
  expect(ok).toBe(true);
});
