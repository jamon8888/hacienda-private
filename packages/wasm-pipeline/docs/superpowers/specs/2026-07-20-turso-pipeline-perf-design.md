...
**Investigation basis:** performance trace of `packages/wasm-pipeline/src/*` + `services/mcp-server/src/*` + `crates/xberg/src/core`, cross-checked against installed `onnxruntime-web@1.26.0`, `gliner@0.0.19`, and **`edgevec@0.9.0`** (README + `edgevec.js` export surface + live browser spike).
...
- Apply extraction / embed / NER tuning so each stage fits a per-stage latency budget.
...
ingestFolder (orchestrator)
  ├─ ExtractStage  ── xberg-wasm  (quality ON: quality_processing=true, WebGPU accel, Rust cache + JS cache, OCR on via strategy)
...
- `packages/wasm-pipeline/src/search/edgevec.ts` — EdgeVec-backed `SearchStore` implementation: dense insert + sparse (`insertSparse`) insert, `hybridSearch` query, **custom binary persistence** (see Section 2), `searchBQ` toggle.
