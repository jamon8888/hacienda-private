...
**Goal:** Rebuild the on-device document pipeline (extract → embed → NER → persist → query) for speed **while keeping EdgeVec** as the browser vector engine. The prior Turso direction was abandoned after a browser-wasm spike (commit `5f43742545`) proved Turso's FTS + vector ANN are hard-gated out of every browser-wasm engine. EdgeVec `0.9.0` already provides ANN + sparse/BM25 + hybrid RRF + binary quantization natively in the browser. The real bottleneck is EdgeVec's broken `save`/`load` (PostCard `WontImplement`), which forced a JSON-localStorage + per-query `insert()` replay. We fix persistence with our own binary IndexedDB store and unlock EdgeVec's native hybrid search.
...
**Tech Stack:** `edgevec@0.9.0` (WASM vector DB: hybrid RRF, sparse BM25, BQ, metadata filter), `onnxruntime-web@1.26.0` (threaded wasm / WebGPU), `gliner@0.0.19` (batched inference), `xberg-wasm` (extraction), vitest (wasm-pipeline tests), `@playwright/test` (browser capability spike).

**Spec:** `docs/superpowers/specs/2026-07-20-turso-pipeline-perf-design.md` (now rewritten for keep-EdgeVec).
...
- `packages/wasm-pipeline/src/ocr.ts` + `ingest.ts` — quality ON (never off) + WebGPU accel, Rust+JS cache, OCR via strategy, token reduction, text-chunk fast-path.
...
- [ ] **Step 1: Write the spike (runs in Playwright Chromium, loads `edgevec.js`)** asserting each required capability + the persistence fix:
  ...
  const did = db.insert(new Float32Array(768).map(() => Math.random()));
...
- [ ] **Step 1: Write failing test** (quality kept ON, WebGPU accel + cache enabled):
  ...
  ```
- [ ] **Step 2: Apply tuning** — `ocr.ts` `defaultExtractionConfig()` (best-quality + real perf levers, verified against `crates/xberg-wasm/src/lib.rs` + `quality_processor.rs`):
  - `cfg.set_enable_quality_processing(true)` — keep best quality (it's the default; never disable — it only adds `quality_score` + light normalization for ~1ms/100KB).
  ...
  - `ingest.ts:50`: `chunkerType` from `scenario` (`markdown` normally, `text` on `lowRam||isMobile`).
