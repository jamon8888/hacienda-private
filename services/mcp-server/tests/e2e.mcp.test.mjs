...
async function main() {
  ...
  const rag = await client.callTool({
    name: "rag_query",
    arguments: { matter_id: matterId, query: "invoice", top_k: 4 },
  });
  ...
  assert(
    JSON.stringify(pii.content).includes("token") || JSON.stringify(pii.content).includes("EMAIL"),
    "list_pii should return token spans",
  );

  // 3. rehydrate_chunk — returns the stored (browser-sealed) ciphertext blob
  const rehyd = await client.callTool({
    name: "rehydrate_chunk",
    arguments: { matter_id: matterId, chunk_id: "sample.txt:0" },
  });
  ...
  assert(
    JSON.stringify(ing.content).includes("folder") || JSON.stringify(ing.content).includes("id"),
    "ingest_folder should create a folder",
  );
  ...
  assert(
    JSON.stringify(red.content).includes("redaction") || JSON.stringify(red.content).includes("id"),
    "redact should record a marker",
  );

  // Forget via HTTP, then a second spawn must see rag_query error not_found
  const http = await fetch(`http://localhost:8787/matters/${matterId}`, {
    method: "DELETE",
